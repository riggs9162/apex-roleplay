...
---------------------------------------------------------------------------*/
local function createShipment(client, args)
	local id = tonumber(args) or -1
	local ent = Entity(id)

	ent = IsValid(ent) and ent or client:GetEyeTrace().Entity
	...
	if not shipID then 
		GAMEMODE:Notify(client, 1, 4, apex.language.GetPhrase("unable", "/makeshipment", ""))
		return
	end
...
apex.commands.Register("/makeshipment", createShipment)

/*---------------------------------------------------------------------------
Split a shipment in two
---------------------------------------------------------------------------*/
local function splitShipment(client, args)
	local id = tonumber(args) or -1
	local ent = Entity(id)
...
apex.commands.Register("/splitshipment", splitShipment)