...
function ENT:Use(client)
	if self.IsBrewing then
		client:Notify("Alcohol brewing is currently at "..self.Percent.."%")
		return
	end

	if not (self.hasYeast or self.hasWater) then
		client:Notify("You have to add yeast and some special water.")
		return
	end
	if not self.hasYeast then
		client:Notify("You have to add yeast to start brewing.")
		return
	end
	if not self.hasWater then
		client:Notify("You have to add some special water start brewing.")
		return
	end
