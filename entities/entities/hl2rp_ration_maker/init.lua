...
net.Receive( "RationMaker", function( len, client )
	...
	if CurTime() - ent.LastSet > 3 then
		...
		for k, v in pairs(spawnedFood) do
			if ( v:GetModel():lower():find("models/weapons/w_package.mdl") ) then
				rations = rations + 1
			end
		end

		local rawrations = ents.FindByClass("hl2rp_rawration")
		if ( table.Count(rawrations) < 2 and rations <= 2 and client:Team() == TEAM_CWU ) then
