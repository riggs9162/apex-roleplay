...
hook.Add("PreDrawHalos", "apex.cp.halo", function()
	local client = LocalPlayer()
	if ( client:Team() == TEAM_CP ) then
		local entities = {}
		for _, ent in ipairs(ents.FindByClass("apex_cp_npc")) do
...
function PANEL:Init()
	if ( IsValid(apex.cp.panel) ) then
		apex.cp.panel:Close()
	end

	apex.cp.panel = self

	self:SetTitle("Civil Protection - Rank and Division Selector")
	...
	self.rankBox:SetTextColor(Color(255, 255, 255))
	self.rankBox:SetValue("Choose a Rank")
	for k, v in ipairs(apex.cp.ranks) do
	...
	self.rankDescription:SetTextColor(Color(255, 255, 255))
	...
	self.rankRequirements:SetTextColor(Color(255, 255, 255))
	...
	self.divisionBox:SetTextColor(Color(255, 255, 255))
	self.divisionBox:SetValue("Choose a Division")
	for k, v in ipairs(apex.cp.divisions) do
	...
	self.divisionDescription:SetTextColor(Color(255, 255, 255))
	...
	self.divisionRequirements:SetTextColor(Color(255, 255, 255))
	...
	self.icon:SetModel("models/dpfilms/metropolice/hdpolice.mdl")
	...
	self.become:SetTextColor(Color(255, 255, 255))
...
function PANEL:Update()
	local rankData = apex.cp.ranks[self.rank]
	if ( rankData ) then
	...
	local divisionData = apex.cp.divisions[self.division]
	...
	if ( rankData and ( self.division == DIVISION_CMD or self.division == DIVISION_SEC ) ) then
...
vgui.Register("apex.cp.menu", PANEL, "DFrame")

net.Receive("apex.cp.menu", function()
	vgui.Create("apex.cp.menu")
end)
