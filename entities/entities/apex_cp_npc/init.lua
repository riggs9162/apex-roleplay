...
util.AddNetworkString("apex.cp.menu")
util.AddNetworkString("apex.cp.select")
...
function ENT:Use(client)
	...
	if ( client:Team() != TEAM_CP ) then
		client:Notify("You are not a member of the Civil Protection.")
		return
	end
...
net.Receive("apex.cp.select", function(_, client)
	if ( !IsValid(client) or !client:IsPlayer() ) then return end

	local rankID = net.ReadUInt(8)
	local divisionID = net.ReadUInt(8)

	local rankData = apex.cp.ranks[rankID]
	local divData = apex.cp.divisions[divisionID]
	if ( !rankData or !divData or client:Team() != TEAM_CP ) then return end
	...
	local jobTitle = "Civil Protection (" .. divData.abbreviation .. " - " .. rankData.abbreviation .. ")"
	local model = divData.model
	local id = math.random(1002, 9900)
	local name = "CP:C17-" .. divData.abbreviation .. "-" .. rankData.abbreviation .. "-" .. id

	if ( divData.noRank ) then
		jobTitle = "Civil Protection (" .. divData.abbreviation .. ")"
		name = "CP:C17-" .. divData.abbreviation .. "-" .. id
	end
