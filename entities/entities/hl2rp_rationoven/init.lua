...
function ENT:SpawnFunction(client, trace)
	...
	e.Owner = client
	e:SetAngles(Angle(0,client:GetAngles().y,0))
	e:Spawn()
	e:Activate()
...
function ENT:Touch( hitEnt )
		...
		hitEnt:SetVelocity(self:GetRight() * (speedamount/10) * -1+ Vector(0, 0, 180))
		elseif !IsValid( hitEnt ) or !hitEnt:GetPhysicsObject():IsValid() then return end
		...
		local AwayVector = Vector(0, 96, 0)

		AwayVector:Rotate(angel)

		local endpoint = self:GetPos()+AwayVector
		debugoverlay.Line(self:GetPos(), endpoint, 0.1, Color(255, 0, 0), true)
		debugoverlay.Text(endpoint, "Ration Exit", 0.1, Color(255, 255, 255), true)
		debugoverlay.Sphere(endpoint, 32, 0.1, Color(255, 0, 0), true)
			if hitEnt:GetPos():Distance(endpoint) < 32 then
