...
function SWEP:PrimaryAttack()
	...
	if SERVER then
		timer.Create("LockPickSounds" .. self:EntIndex(), 1, self.LockPickTime, function()
	...
	elseif CLIENT then
		...
		timer.Create("LockPickDots" .. self:EntIndex(), 0.5, 0, function()
			if not self:IsValid() then timer.Destroy("LockPickDots" .. self:EntIndex()) return end
...
function SWEP:Holster()
	...
	if SERVER then timer.Destroy("LockPickSounds" .. self:EntIndex()) end
	if CLIENT then timer.Destroy("LockPickDots" .. self:EntIndex()) end
	return true
end
...
function SWEP:DrawHUD()
	if self.IsLockPicking then
		...
		local width, height = scrW / 1.5, scrH / 20
		local x, y = scrW / 2 - width / 2, scrH / 1.25 - height
		draw.RoundedBox(0, x, y, width + 10, height, Color(0, 0, 0, 230))
