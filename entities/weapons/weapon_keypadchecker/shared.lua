...
local function getEntityKeypad(ent)
	...
	for k,v in ipairs(ents.FindByClass("sent_keypad")) do
...
function numpad.OnUp(client, key, name, ent, ...)
	numpad.OnUpItems = numpad.OnUpItems or {}
	table.insert(numpad.OnUpItems, {client = client, key = key, name = name, ent = ent, arg = {...}})

	return oldNumpadUp(client, key, name, ent, ...)
end

function numpad.OnDown(client, key, name, ent, ...)
	numpad.OnDownItems = numpad.OnDownItems or {}
	table.insert(numpad.OnDownItems, {client = client, key = key, name = name, ent = ent, arg = {...}})

	return oldNumpadDown(client, key, name, ent, ...)
end
