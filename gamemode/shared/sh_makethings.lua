...
function AddExtraTeam(Name, colorOrTable, model, Description, Weapons, command, maximum_amount_of_this_class, Salary, admin, Vote, Haslicense, NeedToChangeFrom, xp, CustomCheck)
...
function AddAgenda(Title, Manager, Listeners)
	if not Manager then
		hook.Add("PlayerSpawn", "AgendaError", function(client)
		if client:IsAdmin() then client:ChatPrint("WARNING: Agenda made incorrectly, there is no manager! failed to load!") end end)
		return
	end
...
function GM:AddGroupChat(funcOrTeam, ...)
	...
	if type(funcOrTeam) == "function" then
		...
		table.insert(self.DarkRPGroupChats, function(client) return table.HasValue(teams, client:Team()) end)
	end
end
