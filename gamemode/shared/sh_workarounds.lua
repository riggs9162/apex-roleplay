...
function apex.getAvailableVehicles()
...
---------------------------------------------------------------------------*/
hook.Add("PlayerSpawn", "AntiMapKill", function(client)
 --Make so that we can use spawns instead of info_player_start (they will do the same things)
   -- client:SetPos(spawns[ math.random( #spawns ) ]:GetPos());

	timer.Simple(0, function()
		if IsValid(client) and not client:Alive() then
			client:Spawn()
			--local spawns = ents.FindByClass( "info_player_start" )
			--print(spawns[math.random( 1, #spawns )]:GetPos())
			--client:SetPos(spawns[math.random( 1, #spawns )]:GetPos())
			client:AddDeaths(-1)
		end
	end)
end)
...
/*---------------------------------------------------------------------------
Door tool is shitty
Lets fix that huge class exploit
---------------------------------------------------------------------------*/
hook.Add("InitPostEntity", "FixDoorTool", function()
	local oldFunc = makedoor
	if oldFunc then
		function makedoor(client,trace,ang,model,open,close,autoclose,closetime,class,hardware, ...)
			if class != "prop_dynamic" and class != "prop_door_rotating" then return end

			oldFunc(client,trace,ang,model,open,close,autoclose,closetime,class,hardware, ...)
		end
	end
end)
...
hook.Add("CanTool", "DoorExploit", function(client, trace, tool)
	if not IsValid(client:GetActiveWeapon()) or not client:GetActiveWeapon().GetToolObject or not client:GetActiveWeapon():GetToolObject() then return end
