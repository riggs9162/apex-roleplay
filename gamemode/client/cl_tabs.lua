CreateClientConVar("apex_playermodel", "", true, true)
...
/*		local Lockdown = maypanel:Add("DButton")
			Lockdown:SetText(apex.language.GetPhrase("initiate_lockdown"))
			Lockdown.DoClick = function()
				LocalPlayer():ConCommand("apex /lockdown")
			end


			local UnLockdown = maypanel:Add("DButton")
			UnLockdown:SetText(apex.language.GetPhrase("stop_lockdown"))
			UnLockdown.DoClick = function()
				LocalPlayer():ConCommand("apex /unlockdown")
			end

			local Lottery = maypanel:Add("DButton")
			Lottery:SetText(apex.language.GetPhrase("start_lottery"))
			Lottery.DoClick = function()
				LocalPlayer():ConCommand("apex /lottery")
			end
*/
...
local function CPOptns()
	...
	CPCat:SetLabel(team.GetName(LocalPlayer():Team()) .. " options")
		...
		CPpanel:SetSize(740,170)
			...
			Warrant:SetText(apex.language.GetPhrase("searchwarrantbutton"))
			Warrant.DoClick = function()
				local menu = DermaMenu()
				for _,client in player.Iterator() do
					if not LocalPlayer():GetDarkRPVar("wanted") and client != LocalPlayer() then
						menu:AddOption(LocalPlayer():Nick(), function()
							Derma_StringRequest("wanted", "Why would you make "..LocalPlayer():Nick().." wanted?", nil,
								function(a)
									if not IsValid(client) then return end
									RunConsoleCommand("say", "/wanted", LocalPlayer():SteamID64(), a)
								end,
							function() end )
						end)
					end
			...
			UnWarrant:SetText(apex.language.GetPhrase("unwarrantbutton"))
			UnWarrant.DoClick = function()
				local menu = DermaMenu()
				for _,client in player.Iterator() do
					if LocalPlayer():GetDarkRPVar("wanted") and client != LocalPlayer() then
						menu:AddOption(LocalPlayer():Nick(), function() LocalPlayer():ConCommand("apex /unwanted \"" .. LocalPlayer():SteamID64() .. "\"") end)
					end
				end
				menu:Open()
			end
			...
			if LocalPlayer():Team() == TEAM_CHIEF and GAMEMODE.Config.chiefjailpos or LocalPlayer():IsAdmin() then
				...
				SetJailPos:SetText(apex.language.GetPhrase("set_jailpos"))
				SetJailPos.DoClick = function() LocalPlayer():ConCommand("apex /jailpos") end

				local AddJailPos = CPpanel:Add("DButton")
				AddJailPos:SetText(apex.language.GetPhrase("add_jailpos"))
				AddJailPos.DoClick = function() LocalPlayer():ConCommand("apex /addjailpos") end
			end

			local ismayor -- Firstly look if there's a mayor
			local ischief -- Then if there's a chief
			for k,v in player.Iterator() do
				if v:Team() == TEAM_ADMINISTRATOR then
					ismayor = true
					break
				end
			end
			...
			if not ismayor and (Team == TEAM_CHIEF or (not ischief and Team == TEAM_POLICE)) then
				...
				GiveLicense:SetText(apex.language.GetPhrase("give_license_lookingat"))
				GiveLicense.DoClick = function()
					LocalPlayer():ConCommand("apex /givelicense")
				end
			end*/
...
/*		local joblabel = Citpanel:Add("DLabel")
		joblabel:SetText(apex.language.GetPhrase("set_custom_job"))
...
*/
	...
	if LocalPlayer():GetDarkRPVar("inSpawn") == true then
		...
		GoLoyalist.DoClick = function()
			LocalPlayer():ConCommand("apex_citopt 1")
		end
		...
		GoBlackMarket.DoClick = function()
			LocalPlayer():ConCommand("apex_citopt 2")
		end
	end
...
*/
	...
	if LocalPlayer():GetDarkRPVar("inSpawn") == true then
		...
		GoLoyalist:SetTextColor(BColor)
		GoLoyalist.DoClick = function()
			LocalPlayer():ConCommand("apex_citopt 3")
		end
		...
		GoCook.DoClick = function()
			LocalPlayer():ConCommand("apex_citopt 4")
		end
		...
		GoMedic.DoClick = function()
			LocalPlayer():ConCommand("apex_citopt 5")
		end
		...
		GoMedic:SetTextColor(BColor)
		GoMedic.DoClick = function()
			LocalPlayer():ConCommand("apex_citopt 6")
		end
	end
	...
	FirstTabPanel:EnableVerticalScrollbar( true )
		function FirstTabPanel:Update()
			...
			MoneyCat:SetLabel("Tokens")
				...
				GiveMoneyButton:SetText(apex.language.GetPhrase("give_money"))
				GiveMoneyButton:SetTextColor(BColor)
				GiveMoneyButton.DoClick = function()
					Derma_StringRequest("Amount of tokens", "How many tokens do you want to give?", "", function(a) LocalPlayer():ConCommand("say /give " .. tostring(a)) end)
				end

				local SpawnMoneyButton = MoneyPanel:Add("DButton")
				SpawnMoneyButton:SetText(apex.language.GetPhrase("drop_money"))
				SpawnMoneyButton:SetTextColor(BColor)
				SpawnMoneyButton.DoClick = function()
					Derma_StringRequest("Amount of tokens", "How many tokens do you want to drop?", "", function(a) LocalPlayer():ConCommand("say /dropmoney " .. tostring(a)) end)
				end
						...
						if client != LocalPlayer() and IsValid(client) then
							menu:AddOption(LocalPlayer():Nick(), function()
								Derma_StringRequest("Demote reason", "Why would you demote "..LocalPlayer():Nick().."?", nil,
									function(a)
										RunConsoleCommand("say", "/demote", LocalPlayer():SteamID64(), a)
									end,
								function() end )
							end)
						end
...
*/
		...
		elseif LocalPlayer():IsCombine() then
	...
	function hordiv:Update()
		...
		local function AddIcon(Model, name, description, Weapons, command, special, specialcommand, XPR)
			...
			icon.OnCursorEntered = function()
				...
				Info[1] = apex.language.GetPhrase("job_name") .. name
				Info[2] = apex.language.GetPhrase("job_description") .. description
				Info[3] = ""
				model = IconModel
				UpdateInfo()
			end
			...
			icon.DoClick = function()
				...
				if type(Model) == "table" and #Model > 0 then
					...
					for k,v in pairs(Model) do
						...
						icon:SetToolTip()
						icon.DoClick = function()
							LocalPlayer():ConCommand("apex_playermodel" .. v)
							LocalPlayer():ConCommand("_rp_ChosenModel" .. v)
							DoChatCommand(frame)
						end
					end
				else
		...
		local function UnAddIcon(Model, name, description, Weapons, command, special, specialcommand, XPR)
			...
			unicon.DoClick = function()
				local function DoChatCommand(frame)
					if special then
						local menu = DermaMenu()
						menu:AddOption("Vote", function() LocalPlayer():ConCommand("apex "..command) frame:Close() end)
						menu:AddOption("Do not vote", function() LocalPlayer():ConCommand("apex " .. specialcommand) frame:Close() end)
						menu:Open()
					else
						if string.sub(command, 1, 1) == "/" then
							LocalPlayer():ConCommand("say " .. command)
						else
							LocalPlayer():ConCommand("apex " .. command)
						end
				...
				AmmPanel:EnableVerticalScrollbar(true)
					...
					for k,v in pairs(GAMEMODE.AmmoTypes) do
						if not v.customCheck or v.customCheck(LocalPlayer()) then
							AddAmmoIcon(v.model, apex.language.GetPhrase("buy_a", v.name, GAMEMODE.Config.currency .. v.price), "/buyammo " .. v.ammoType)
							ammnum = ammnum + 1
						end
					end
				...
				WepPanel:EnableVerticalScrollbar(true)
					...
					for k,v in pairs(CustomShipments) do
						...
						if (v.seperate and (not GAMEMODE.Config.restrictbuypistol or
							...
							AddIcon(v.model, apex.language.GetPhrase("buy_a", "a "..v.name, GAMEMODE.Config.currency..(v.pricesep or "")), "/buy "..v.name)
							wepnum = wepnum + 1
						end
					end
				...
				ShipPanel:EnableVerticalScrollbar(true)
					...
					for k,v in pairs(CustomShipments) do
						...
						if not v.noship and table.HasValue(v.allowed, LocalPlayer():Team())
							and (not v.customCheck or (v.customCheck and v.customCheck(LocalPlayer()))) then
							AddShipIcon(v.model, apex.language.GetPhrase("buy_a", "a "..v.name .." shipment", GAMEMODE.Config.currency .. tostring(v.price)), "/buyshipment "..v.name)
							shipnum = shipnum + 1
						end
					end
				if shipnum != 0 then
...
function CanBuyFood(client)
	if client:Team() == TEAM_CP and client.DarkRPVars.Division and client.DarkRPVars.Division == 2 then
		return true
	elseif client:Team() == TEAM_CWU and client:GetDarkRPVar("citopt") and client:GetDarkRPVar("citopt") == 2 then
		return true
					...
					if FoodItems and CanBuyFood(LocalPlayer()) then
						for k,v in pairs(FoodItems) do
							if v.team == LocalPlayer():Team() then
								AddEntIcon(v.model, apex.language.GetPhrase("buy_a", "a "..k, "T"..v.price..""), "/buyfood "..k)
								entnum = entnum + 1
							end
						end
					end
			...
			VehicleCat:SetLabel("Vehicles")
				...
				for k,v in pairs(CustomVehicles) do
					if (not v.allowed or table.HasValue(v.allowed, LocalPlayer():Team())) and (not v.customCheck or v.customCheck(LocalPlayer())) then
						local Skin = (apex.getAvailableVehicles()[v.name] and apex.getAvailableVehicles()[v.name].KeyValues and apex.getAvailableVehicles()[v.name].KeyValues.Skin) or "0"
