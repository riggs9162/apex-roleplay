...
local function DoVoteAnswerQuestion(client, cmd, args)
...
concommand.Add("apex_vote", DoVoteAnswerQuestion)
...
local function ChangeJobVGUI()
	if not F4Menu or not F4Menu:IsValid() then
		...
		F4Menu.Paint = function( self, w, h ) -- 'function Frame:Paint( w, h )' works too
			draw.RoundedBox( 0, 0, 0, w, h, Color( 0,0,0, 200 ) )
			draw.RoundedBox( 0, 0, 0, w, 25, Color( 73,147,197, 150 ) ) -- Draw a red box instead of the frame
		end
		GAMEMODE:addF4MenuTab("Tokens/Commands", GAMEMODE:MoneyTab(), "icon16/money.png")
    ...
    --		if (!v.Tab) then continue end
         ...
         if v.Tab == F4MenuTabs:GetActiveTab() then
...
local function KeysMenu()
	...
	if trace.Entity:OwnedBy(LocalPlayer()) then
		...
		AddOwner.DoClick = function()
			...
			for k,v in player.Iterator() do
				if not trace.Entity:OwnedBy(v) and not trace.Entity:AllowedToOwn(v) then
					menu.found = true
					menu:AddOption(v:Nick(), function() LocalPlayer():ConCommand("say /ao " .. v:SteamID64()) end)
				end
			end
		...
		RemoveOwner.DoClick = function()
			...
			for k,v in player.Iterator() do
				if (trace.Entity:OwnedBy(v) and not trace.Entity:IsMasterOwner(v)) or trace.Entity:AllowedToOwn(v) then
					menu.found = true
					menu:AddOption(v:Nick(), function() LocalPlayer():ConCommand("say /ro " .. v:SteamID64()) end)
				end
			end
			if not menu.found then
				menu:AddOption("Noone available", function() end)
		...
		if (FAdmin and FAdmin.Access.PlayerHasPrivilege(LocalPlayer(), "apex_doorManipulation") or LocalPlayer():IsAdmin()) and not Vehicle then
			...
			SetCopsOnly.DoClick = function()
				...
				menu:AddOption("None", function() LocalPlayer():ConCommand("say /togglegroupownable") Frame:Close() end)
				for k,v in pairs(RPExtraTeamDoors) do
					groups:AddOption(k, function() LocalPlayer():ConCommand("say /togglegroupownable " .. k) Frame:Close() end)
				end
				...
				for k,v in pairs(RPExtraTeams) do
					if not trace.Entity.DoorData.TeamOwn or not trace.Entity.DoorData.TeamOwn[k] then
						add:AddOption( v.name, function() LocalPlayer():ConCommand("say /toggleteamownable " .. k) Frame:Close() end )
					else
						remove:AddOption( v.name, function() LocalPlayer():ConCommand("say /toggleteamownable " .. k) Frame:Close() end )
					end
				end
	...
	elseif not trace.Entity:OwnedBy(LocalPlayer()) and trace.Entity:IsOwnable() and not trace.Entity:IsOwned() and not trace.Entity.DoorData.NonOwnable then
		...
		if (FAdmin and FAdmin.Access.PlayerHasPrivilege(LocalPlayer(), "apex_doorManipulation") or LocalPlayer():IsAdmin()) then
	...
	elseif (FAdmin and FAdmin.Access.PlayerHasPrivilege(LocalPlayer(), "apex_doorManipulation") or LocalPlayer():IsAdmin()) and trace.Entity.DoorData.NonOwnable then
		Frame:SetSize(200, 250)
	...
	elseif (FAdmin and FAdmin.Access.PlayerHasPrivilege(LocalPlayer(), "apex_doorManipulation") or LocalPlayer():IsAdmin()) and not trace.Entity:OwnedBy(LocalPlayer()) and trace.Entity:IsOwned() and not trace.Entity:AllowedToOwn(LocalPlayer()) then
		Frame:SetSize(200, 250)
		local DisableOwnage = vgui.Create("DButton", Frame)
...
-- Pressing F2 will open the keys menu
hook.Add("PlayerButtonDown", "OpenKeysMenu", function(ply, button)
...
net.Receive("DarkRP_KeysMenu", function()
	KeysMenu()
end)