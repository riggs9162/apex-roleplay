...
function FPP.Notify(client, text, bool)
	if client:EntIndex() == 0 then
		ServerLog(text)
		return
	end
	umsg.Start("FPP_Notify", client)
		umsg.String(text)
		umsg.Bool(bool)
	umsg.End()
...
local function FPP_SetSetting(client, cmd, args)
	if client:EntIndex() != 0 and not client:IsSuperAdmin() then FPP.Notify(client, "You need superadmin privileges in order to be able to use this command", false) return end
	if not args[1] or not args[3] or not FPP.Settings[args[1]] then FPP.Notify(client, "Argument(s) invalid", false) return end
	if not FPP.Settings[args[1]][args[2]] then FPP.Notify(client, "Argument invalid",false) return end
	...
	MySQLite.queryValue("SELECT var FROM ".. args[1] .. " WHERE var = "..sql.SQLStr(args[2])..";", function(data)
		...
		FPP.NotifyAll(((client.Nick and client:Nick()) or "Console").. " set ".. string.lower(string.gsub(args[1], "FPP_", "")) .. " "..args[2].." to " .. tostring(args[3]), util.tobool(tonumber(args[3])))
	end)
end
concommand.Add("FPP_setting", FPP_SetSetting)

local function AddBlocked(client, cmd, args)
	if client:EntIndex() != 0 and not client:IsSuperAdmin() then FPP.Notify(client, "You need superadmin privileges in order to be able to use this command", false) return end
	if not args[1] or not args[2] or not FPP.Blocked[args[1]] then FPP.Notify(client, "Argument(s) invalid", false) return end
	...
	MySQLite.query("SELECT * FROM FPP_BLOCKED1;", function(data)
		...
		FPP.NotifyAll(((client.Nick and client:Nick()) or "Console").. " added ".. args[2] .. " to the "..args[1] .. " black/whitelist", true)
	end)
end
concommand.Add("FPP_AddBlocked", AddBlocked)

local function AddBlockedModel(client, cmd, args)
	if client:EntIndex() != 0 and not client:IsSuperAdmin() then FPP.Notify(client, "You need superadmin privileges in order to be able to use this command", false) return end
	if not args[1] then FPP.Notify(client, "Argument(s) invalid", false) return end

	local model = string.lower(args[1])
	model = string.Replace(model, "\\", "/")

	if FPP.BlockedModels[model] then FPP.Notify(client, "This model is already in the black/whitelist", false) return end

	FPP.BlockedModels[model] = true
	MySQLite.query("REPLACE INTO FPP_BLOCKEDMODELS1 VALUES("..sql.SQLStr(model)..");")

	FPP.NotifyAll(((client.Nick and client:Nick()) or "Console").. " added ".. model .. " to the blocked models black/whitelist", true)
end
concommand.Add("FPP_AddBlockedModel", AddBlockedModel)

local function RemoveBlocked(client, cmd, args)
	if client:EntIndex() != 0 and not client:IsSuperAdmin() then FPP.Notify(client, "You need superadmin privileges in order to be able to use this command", false) return end
	if not args[1] or not args[2] or not FPP.Blocked[args[1]] then FPP.Notify(client, "Argument(s) invalid", false) return end
	...
	FPP.NotifyAll(((client.Nick and client:Nick()) or "Console").. " removed ".. args[2] .. " from the "..args[1] .. " black/whitelist", false)
end
concommand.Add("FPP_RemoveBlocked", RemoveBlocked)

local function RemoveBlockedModel(client, cmd, args)
	if client:EntIndex() != 0 and not client:IsSuperAdmin() then FPP.Notify(client, "You need superadmin privileges in order to be able to use this command", false) return end
	if not args[1] then FPP.Notify(client, "Argument(s) invalid", false) return end
	local model = string.lower(args[1])
	...
	FPP.NotifyAll(((client.Nick and client:Nick()) or "Console").. " removed ".. model .. " from the blocked models black/whitelist", false)
end
...
local function ShareProp(client, cmd, args)
	if not args[1] or not IsValid(Entity(args[1])) or not args[2] then FPP.Notify(client, "Argument(s) invalid", false) return end
	local ent = Entity(args[1])

	if not FPP.PlayerCanTouchEnt(client, ent, "Toolgun1", "FPP_TOOLGUN1", true) then --Note: This returns false when it's someone elses shared entity, so that's not a glitch
		FPP.Notify(client, "You do not have the right to share this entity.", false)
		return
	end
	...
	if not tonumber(args[2]) or not IsValid(Player(tonumber(args[2]))) then -- This is for sharing prop per utility
		if not allowedShares[args[2]] then
			FPP.Notify(client, "Argument(s) invalid", false)
			return
		end
		...
		if not ent.AllowedPlayers and toggle then -- Make the table if it isn't there
			...
			if toggle and not table.HasValue(ent.AllowedPlayers, target) then
				...
				FPP.Notify(target, client:Nick().. " shared an entity with you!", true)
			elseif not toggle then
				for k,v in pairs(ent.AllowedPlayers) do
					if v == target then
						table.remove(ent.AllowedPlayers, k)
						FPP.Notify(target, client:Nick().. " unshared an entity with you!", false)
					end
				end
			end
		end
...
local function SendSettings(client)
...
local function AddGroup(client, cmd, args)
	if client:EntIndex() != 0 and not client:IsSuperAdmin() then FPP.Notify(client, "You need superadmin privileges in order to be able to use this command", false) return end
	if not args[1] then FPP.Notify(client, "Invalid argument(s)", false) return end-- Args: 1 = name, optional: 2 = allowdefault
	local name = string.lower(args[1])
	local allowdefault = tonumber(args[2]) or 1

	if FPP.Groups[name] then
		FPP.Notify(client, "Group already exists", false)
		return
	end
	...
	FPP.Notify(client, "Group added succesfully", true)
end
...
local function RemoveGroup(client, cmd, args)
	if client:EntIndex() != 0 and not client:IsSuperAdmin() then FPP.Notify(client, "You need superadmin privileges in order to be able to use this command", false) return end
	if not args[1] then FPP.Notify(client, "Invalid argument(s)", false) return end-- Args: 1 = name
	local name = string.lower(args[1])

	if not FPP.Groups[name] then
		FPP.Notify(client, "Group does not exists", false)
		return
	end

	if name == "default" then
	FPP.Notify(client, "Can not remove default group", false)
		return
	end
	...
	FPP.Notify(client, "Group removed succesfully", true)
end
concommand.Add("FPP_RemoveGroup", RemoveGroup)

local function GroupChangeAllowDefault(client, cmd, args)
	if client:EntIndex() != 0 and not client:IsSuperAdmin() then FPP.Notify(client, "You need superadmin privileges in order to be able to use this command", false) return end
	if not args[2] then FPP.Notify(client, "Invalid argument(s)", false) return end-- Args: 1 = groupname, 2 = new value 1/0
	...
	FPP.Notify(client, "Group status changed succesfully", true)
end
concommand.Add("FPP_ChangeGroupStatus", GroupChangeAllowDefault)

local function GroupAddTool(client, cmd, args)
	if client:EntIndex() != 0 and not client:IsSuperAdmin() then FPP.Notify(client, "You need superadmin privileges in order to be able to use this command", false) return end
	if not args[2] then FPP.Notify(client, "Invalid argument(s)", false) return end-- Args: 1 = groupname, 2 = tool
	...
	if table.HasValue(FPP.Groups[name].tools, tool) then
		FPP.Notify(client, "Tool is already in group!", false)
		return
	end
	...
	FPP.Notify(client, "Tool added succesfully", true)
end
concommand.Add("FPP_AddGroupTool", GroupAddTool)

local function GroupRemoveTool(client, cmd, args)
	if client:EntIndex() != 0 and not client:IsSuperAdmin() then FPP.Notify(client, "You need superadmin privileges in order to be able to use this command", false) return end
	if not args[2] then FPP.Notify(client, "Invalid argument(s)", false) return end-- Args: 1 = groupname, 2 = tool
	...
	if not table.HasValue(FPP.Groups[name].tools, tool) then
		FPP.Notify(client, "Tool does not exist in group!", false)
		return
	end
	...
	FPP.Notify(client, "Tool removed succesfully", true)
end
concommand.Add("FPP_RemoveGroupTool", GroupRemoveTool)

local function PlayerSetGroup(client, cmd, args)
	if client:EntIndex() != 0 and not client:IsSuperAdmin() then FPP.Notify(client, "You need superadmin privileges in order to be able to use this command", false) return end
	if not args[2] then FPP.Notify(client, "Invalid argument(s)", false) return end-- Args: 1 = player, 2 = group

	local name = args[1]
	local group = string.lower(args[2])
	if IsValid(Player(tonumber(name) or 0)) then name = Player(tonumber(name)):SteamID64()
	elseif not string.find(name, "STEAM") and name != "UNKNOWN" then FPP.Notify(client, "Invalid argument(s)", false) return end
	...
	FPP.Notify(client, "Player group succesfully set", true)
end
concommand.Add("FPP_SetPlayerGroup", PlayerSetGroup)

local function SendGroupData(client, cmd, args)
...
local function SendGroupMemberData(client, cmd, args)
	-- Need superadmin so clients can't spam this on server
	if client:EntIndex() != 0 and not client:IsSuperAdmin() then FPP.Notify(client, "You need superadmin privileges in order to be able to use this command", false) return end
...
local function SendBlocked(client, cmd, args)
	if not args[1] or not FPP.Blocked[args[1]] then return end

	client.FPPUmsg1 = client.FPPUmsg1 or {}
	client.FPPUmsg1[args[1]] = client.FPPUmsg1[args[1]] or 0
	if client.FPPUmsg1[args[1]] > CurTime() - 5 then return end
	client.FPPUmsg1[args[1]] = CurTime()

	for k,v in pairs(FPP.Blocked[args[1]]) do
		umsg.Start("FPP_blockedlist", client)
			umsg.String(args[1])
			umsg.String(k)
		umsg.End()
	end
...
local function SendBlockedModels(client, cmd, args)
	client.FPPUmsg2 = client.FPPUmsg2 or 0
	if client.FPPUmsg2 > CurTime() - 10 then return end
	client.FPPUmsg2 = CurTime()

	local i = 0
	for k,v in pairs(FPP.BlockedModels) do
		timer.Simple(i*0.01, function()
			umsg.Start("FPP_BlockedModel", client)
				umsg.String(k)
			umsg.End()
		end)
	end
...
local function SendRestrictedTools(client, cmd, args)
	client.FPPUmsg3 = client.FPPUmsg3 or 0
	if client.FPPUmsg3 > CurTime() - 5 then return end
	client.FPPUmsg3 = CurTime()

	if not args[1] then return end
	umsg.Start("FPP_RestrictedToolList", client)
		umsg.String(args[1])
...
--Buddies!
local function SetBuddy(client, cmd, args)
	if not args[6] then FPP.Notify(client, "Argument(s) invalid", false) return end
	local buddy = tonumber(args[1]) and Player(tonumber(args[1]))
	if not IsValid(buddy) then FPP.Notify(client, "Player invalid", false) return end

	client.Buddies = client.Buddies or {}
	for k,v in pairs(args) do args[k] = tonumber(v) end
	client.Buddies[buddy] = {physgun1 = util.tobool(args[2]), gravgun1 = util.tobool(args[3]), toolgun1 = util.tobool(args[4]), playeruse1 = util.tobool(args[5]), entitydamage1 = util.tobool(args[6])}
end
concommand.Add("FPP_SetBuddy", SetBuddy)

local function CleanupDisconnected(client, cmd, args)
	if client:EntIndex() != 0 and not client:IsAdmin() then FPP.Notify(client, "You can't clean up", false) return end
	...
	FPP.NotifyAll(((client.Nick and client:Nick()) or "Console") .. " removed "..Player(args[1]):Nick().. "'s entities", true)
end
concommand.Add("FPP_Cleanup", CleanupDisconnected)

local function SetToolRestrict(client, cmd, args)
	if client:EntIndex() != 0 and not client:IsSuperAdmin() then FPP.Notify(client, "You can't set tool restrictions", false) return end
	if not args[3] then FPP.Notify(client, "Invalid argument(s)", false) return end--FPP_restricttool <toolname> <type(admin/team)> <toggle(1/0)>
	...
	if RestrictWho == "admin" then
		...
		FPP.NotifyAll(((client.Nick and client:Nick()) or "Console") .. " changed the admin status of " .. toolname , true)
	elseif RestrictWho == "team" then
		...
		if tobool(teamtoggle) then -- if the team restrict is enabled
			FPP.NotifyAll(((client.Nick and client:Nick()) or "Console") .. " restricted " .. toolname .. " to certain teams", true)
			...
			FPP.NotifyAll(((client.Nick and client:Nick()) or "Console") .. " removed teamrestrictions from " .. toolname, true)
...
local function RestrictToolPerson(client, cmd, args)
	if client:EntIndex() != 0 and not client:IsSuperAdmin() then return end
	if not args[3] then FPP.Notify(client, "Invalid argument(s)", false) return end--FPP_restricttoolperson <toolname> <userid> <disallow, allow, remove(0,1,2)>

	local toolname = args[1]
	local target = Player(tonumber(args[2]))
	local access = tonumber(args[3])
	if not target:IsValid() then FPP.Notify(client, "Invalid argument(s)", false) return end
	if access < 0 or access > 2 then FPP.Notify(client, "Invalid argument(s)", false) return end
	...
	if access == 0 or access == 1 then -- Disallow, even if other people can use it
		FPP.RestrictedToolsPlayers[toolname][target:SteamID64()] = access == 1

		MySQLite.query("REPLACE INTO FPP_TOOLRESTRICTPERSON1 VALUES("..sql.SQLStr(toolname)..", "..sql.SQLStr(target:SteamID64())..", ".. access ..");")
	elseif access == 2 then -- reset tool status(make him like everyone else)
		FPP.RestrictedToolsPlayers[toolname][target:SteamID64()] = nil
		MySQLite.query("DELETE FROM FPP_TOOLRESTRICTPERSON1 WHERE toolname = "..sql.SQLStr(toolname).." AND steamid = "..sql.SQLStr(target:SteamID64())..";")
	end
end
