...
function FPP.LoadBuddies()
	...
	if data then
		for k,v in pairs(data) do
			...
			for num,client in player.Iterator() do --If the buddies are in the server then add them serverside
				if client:SteamID64() == v.steamid then
					sql.Query("UPDATE FPP_Buddies SET name = "..sql.SQLStr(client:Nick()).." WHERE steamid = ".. sql.SQLStr(v.steamid) ..";") -- update the name
					FPP.Buddies[v.steamid].name = client:Nick()
					RunConsoleCommand("FPP_SetBuddy", client:UserID(), v.physgun, v.gravgun, v.toolgun, v.playeruse, v.entitydamage)
				end
			end
		end
	end
...
function FPP.SaveBuddy(SteamID, Name, Type, value)
	...
	for k,v in player.Iterator() do
		if v:SteamID64() == SteamID then -- If the person you're adding is in the server then add him serverside
	...
	if data then
		for k,v in pairs(data) do
			if v.steamid == SteamID then--make the player buddy if he's in your buddies list
				RunConsoleCommand("FPP_SetBuddy", client:UserID(), v.physgun, v.gravgun, v.toolgun, v.playeruse, v.entitydamage)
				sql.Query("UPDATE FPP_Buddies SET name = "..sql.SQLStr(client:Nick()).." WHERE steamid = ".. sql.SQLStr(SteamID) ..";") -- update the name
				FPP.Buddies[SteamID] = FPP.Buddies[SteamID] or {}
				FPP.Buddies[SteamID].name = client:Nick()
			end
		end
	end
end
