...
apex.latestNameChanges = apex.latestNameChanges or {}
local function RPName(client, args)
	local canChangeName, reason = hook.Call("CanChangeRPName", GAMEMODE, client, string.lower(args))
	if ( canChangeName == false ) then
		client:Notify(reason)
		return ""
	end

	client:SetRPName(args)
	apex.latestNameChanges[client] = {
		name = args,
		started = CurTime(),
		duration = 60
	}

	client:Notify("Your RP name has been set to '" .. args .. "'.")

	return ""
end

apex.commands.Register("/rpname", RPName)
apex.commands.Register("/name", RPName)
apex.commands.Register("/nick", RPName)

function meta:IsDispatch()
	if ( self:IsCombine() ) then
		if ( self:Team() == TEAM_DISPATCH ) then
			return true
		end

		local division, rank = self:GetDarkRPVar("Division"), self:GetDarkRPVar("Rank")
		if ( division and ( division == 7 or division == 8 ) ) or ( rank and rank == 9 ) then
			return true
		end
	end
	...
	apex.db.RetrieveRPNames(self, name, function(taken)
		...
		if taken then
			if firstRun then
				...
				apex.db.StoreRPName(self, name .. " 1")
				GAMEMODE:Notify(self, 0, 12, "Someone is already using your Steam name as their RP name so we gave you a '1' after your name.")
			else
				GAMEMODE:Notify(self, 1, 5, apex.language.GetPhrase("unable", "RPname", "it's been taken"))
				return ""
			end
		else
			if not firstRun then -- Don't save the steam name in the database
				GAMEMODE:NotifyAll(2, 6, apex.language.GetPhrase("rpname_changed", self:SteamName(), name))
				apex.db.StoreRPName(self, name)
			end
		end
	end)
end
...
function meta:RestorePlayerData()
	...
	apex.db.RetrievePlayerData(self, function(data)
		if not IsValid(self) then return end
...
function meta:InitiateTax()
	...
	timer.Create("apex_tax_"..uniqueid, taxtime or 600, 0, function()
		if not IsValid(self) then
			timer.Destroy("apex_tax_"..uniqueid)
			return
		end
...
function meta:TeamBan(t, time)
	...
	timer.Simple(time or GAMEMODE.Config.firetime, function()
		if not IsValid(self) then return end
		self:TeamUnBan(t)
	end)
end
...
return false
	...
	if self:IsArrested() and not force then
		GAMEMODE:Notify(self, 1, 4, apex.language.GetPhrase("unable", team.GetName(t), ""))
		return false
	end
	...
	if t != GAMEMODE.DefaultTeam and not self:ChangeAllowed(t) and not force then
		self:Notify(apex.language.GetPhrase("unable", team.GetName(t), "banned/fired"))
		return false
	end

	if !self:IsAdmin() and self.LastJob and jobdelay - (CurTime() - self.LastJob) >= 0 and not force then
		self:Notify(apex.language.GetPhrase("have_to_wait",  math.ceil(jobdelay - (CurTime() - self.LastJob)), "/job"))
		return false
	end
	...
	if self.IsBeingDemoted then
		...
		GAMEMODE:Notify(self, 1, 4, "You tried to escape demotion. You failed, and have been fired.")
	...
	if TEAM.customCheck and not TEAM.customCheck(self) then
		GAMEMODE:Notify(self, 1, 4, TEAM.CustomCheckFailMsg or apex.language.GetPhrase("unable", team.GetName(t), ""))
		return false
	end

	if not self.DarkRPVars["Priv"..TEAM.command] and not force and not self:IsVIP() then
		if type(TEAM.NeedToChangeFrom) == "number" and prevTeam != TEAM.NeedToChangeFrom then
			GAMEMODE:Notify(self, 1,4, apex.language.GetPhrase("need_to_be_before", team.GetName(TEAM.NeedToChangeFrom), TEAM.name))
			return false
		...
		max < 1 and (team.NumPlayers(t) + 1) / #player.GetAll() > max) then -- fractional limit (in percentages)
			self:Notify(apex.language.GetPhrase("team_limit_reached", TEAM.name))
			return false
		end
	end

	--2/5 CP max.
	if t == TEAM_CP and not (force or self:IsVIP()) then
		--print("Test")
		if table.Count(team.GetPlayers(TEAM_CP)) >= roundUp(table.Count(player.GetAll())*(2/5)+1) then

			self:Notify("Max Civil Protection hit currently max: "..roundUp(table.Count(player.GetAll())*(2/5)+1))
			return
		end
...
-- Check for XP Requirement

	xp = self:GetXP() or 0
	--print(tostring(xp).." : "..TEAM.xp)
	XPR = tonumber(TEAM.xp)

	if(XPR > xp and not force)then
		--print("Denied.")
		self:Notify("You do not have the required XP to join "..TEAM.name)
		return false
	end
	...
	--GAMEMODE:NotifyAll(0, 4, apex.language.GetPhrase("job_has_become", self:Nick(), TEAM.name))
	...
	if GAMEMODE.Config.removeclassitems then
		for k, v in ipairs(ents.FindByClass("microwave")) do
		...
		for k, v in ipairs(ents.FindByClass("gunlab")) do
			if v.allowed and type(v.allowed) == "table" and table.HasValue(v.allowed, t) then continue end
			if v.SID == self.SID then v:Remove() end
		end

		for k, v in ipairs(ents.FindByClass("drug_lab")) do
			if v.allowed and type(v.allowed) == "table" and table.HasValue(v.allowed, t) then continue end
			if v.SID == self.SID then v:Remove() end
		end
	...
	apex.db.Log(self:Nick().." ("..self:SteamID64()..") changed to "..team.GetName(t), nil, Color(100, 0, 255))
...
function meta:AddMoney(amount)
	...
	apex.db.StoreMoney(self, total)
end

function meta:PayDay()
	if not IsValid(self) then return end
	if not self:IsArrested() then
		apex.db.RetrieveSalary(self, function(amount)
			...
			if amount == 0 or not amount then
				// self:Notify( apex.language.GetPhrase("payday_unemployed"))
			else
				if not self:GetPData( "bankmoney") then self:SetPData( "bankmoney", 10) end
				self:SetPData( "bankmoney", self:GetPData( "bankmoney" ) + amount )
				self:Notify( apex.language.GetPhrase("payday_message", GAMEMODE.Config.currency .. amount))
			end
		end)
	else
		self:Notify( apex.language.GetPhrase("payday_missed"))
	end
end
...
function meta:DoPropertyTax()
	...
	if self:IsCombine() and GAMEMODE.Config.cit_propertytax then return end
	...
	if self:CanAfford(tax) then
		if tax != 0 then
			self:AddMoney(-tax)
			GAMEMODE:Notify(self, 0, 5, apex.language.GetPhrase("property_tax", GAMEMODE.Config.currency .. tax))
		end
	else
		GAMEMODE:Notify(self, 1, 8, apex.language.GetPhrase("property_tax_cant_afford"))
		self:UnownAll()
	end
end
...
function meta:DropDRPWeapon(weapon)
	...
	local ammo = self:GetAmmoCount(weapon:GetPrimaryAmmoType()) or false
...
/*---------------------------------------------------------
 Jail/arrest
 ---------------------------------------------------------*/
local function JailPos(client)
	-- Admin or Chief can set the Jail Position
	if client:HasPriv("apex_commands") then
		apex.db.StoreJailPos(client)
	else
		...
		GAMEMODE:Notify(client, 1, 4, str)
	end
	return ""
end
--apex.commands.Register("/jailpos", JailPos)

local function AddJailPos(client)
	-- Admin or Chief can add Jail Positions
	if client:HasPriv("apex_commands") then
		apex.db.StoreJailPos(client, true)
	else
		local str = apex.language.GetPhrase("admin_only")
		if GAMEMODE.Config.chiefjailpos then
			str = apex.language.GetPhrase("chief_or") .. str
		end
...
--apex.commands.Register("/addjailpos", AddJailPos)