...
local function ccDoVote(client, cmd, args)
	local vote = Votes[tonumber(args[1] or 0)]

	if not vote then return end
	if args[2] != "yea" and args[2] != "nay" then return end

	local canVote, message = hook.Call("CanVote", GAMEMODE, client, vote)

	if vote.voters[client] or vote.exclude[client] or canVote == false then
		GAMEMODE:Notify(client, 1, 4, message or "You cannot vote!")
		return
	end
	vote.voters[client] = true

	vote:handleNewVote(client, args[2])
end
concommand.Add("vote", ccDoVote)

function Vote:handleNewVote(client, choice)
...
function GM.vote:create(question, voteType, target, time, callback, excludeVoters, fail)
	...
	if #player.GetAll() <= table.Count(excludeVoters) then
		GAMEMODE:Notify(target, 0, 4, apex.language.GetPhrase("vote_alone"))
		newvote:callback(1)
		return
	end
