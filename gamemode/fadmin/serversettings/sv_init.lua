...
local function ServerSetting(client, cmd, args)
	if not FAdmin.Access.PlayerHasPrivilege(client, "ServerSetting") then FAdmin.Messages.SendMessage(client, 5, "No access!") return end
	if not args[2] then FAdmin.Messages.SendMessage(client, 5, "Incorrect argument") return end
	...
	FAdmin.Messages.ActionMessage(client, player.GetAll(), "You have set ".. args[1].. " to ".. unpack(CommandArgs),
