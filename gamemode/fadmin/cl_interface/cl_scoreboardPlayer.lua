...
function FAdmin.ScoreBoard.Player.Show(client)
	local OldPly = FAdmin.ScoreBoard.Player.Player
	client = client or FAdmin.ScoreBoard.Player.Player
	FAdmin.ScoreBoard.Player.Player = client

	if not IsValid(client) or not IsValid(FAdmin.ScoreBoard.Player.Player) then FAdmin.ScoreBoard.ChangeView("Main") return end
	...
	FAdmin.ScoreBoard.Player.Controls.AvatarBackground:SetPlayer(client, 184)
	FAdmin.ScoreBoard.Player.Controls.AvatarBackground:SetVisible(true)

	if FAdmin.ScoreBoard.Player.Controls.AvatarLarge and FAdmin.ScoreBoard.Player.Controls.AvatarLarge:IsValid() and FAdmin.ScoreBoard.Player.Controls.AvatarLarge.Player == client then
	...
	for k, v in pairs(FAdmin.ScoreBoard.Player.Information) do
		...
		if Value and Value != "" then
			...
			timer.Create("FAdmin_Scoreboard_text_update_"..v.name, 1, 0, function()
				if not IsValid(client) or not IsValid(FAdmin.ScoreBoard.Player.Player) or not ValidPanel(Text) then
					timer.Destroy("FAdmin_Scoreboard_text_update_"..v.name)
					if FAdmin.ScoreBoard.Visible and (not IsValid(client) or not IsValid(FAdmin.ScoreBoard.Player.Player)) then FAdmin.ScoreBoard.ChangeView("Main") end
					return
				end
	...
	FAdmin.ScoreBoard.Player.Controls.ButtonPanel:SetMouseInputEnabled( true )
	...
	for k,v in ipairs(FAdmin.ScoreBoard.Player.ActionButtons) do
		if v.Visible == true or (type(v.Visible) == "function" and v.Visible(FAdmin.ScoreBoard.Player.Player) == true) then
			...
			elseif type(v.Image) == "function" then
				local img1, img2 = v.Image(client)
...
FAdmin.ScoreBoard.Player:AddInformation("RP Name", function(client) return client:Nick() end)
FAdmin.ScoreBoard.Player:AddInformation("SteamID", function(client) return client:SteamID64() end)
FAdmin.ScoreBoard.Player:AddInformation("Kills", function(client) return client:Frags() end)
FAdmin.ScoreBoard.Player:AddInformation("Deaths", function(client) return client:Deaths() end)
FAdmin.ScoreBoard.Player:AddInformation("Health", function(client) return client:Health() end)
FAdmin.ScoreBoard.Player:AddInformation("Ping", function(client) return client:Ping() end)
