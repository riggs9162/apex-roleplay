local function SetHealth(client, cmd, args)
	...
	for _, target in pairs(targets) do
		if not FAdmin.Access.PlayerHasPrivilege(client, "SetHealth", target) then FAdmin.Messages.SendMessage(client, 5, "No access!") return end
		if IsValid(target) then
			target:SetHealth(Health)
		end
	end

	FAdmin.Messages.ActionMessage(client, targets, "You've set the health of %s to ".. Health, "Your health was set by %s", "Set the health of %s to ".. Health)
end
