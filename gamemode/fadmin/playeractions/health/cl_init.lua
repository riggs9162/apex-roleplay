FAdmin.StartHooks["Health"] = function()
	...
	function(client) return FAdmin.Access.PlayerHasPrivilege(LocalPlayer(), "SetHealth", client) end,
	function(client, button)
		--Do nothing when the button has been clicked
	end,
	function(client, button) -- Create the Wang when the mouse is pressed
		button.OnMousePressed = function()
			local window = Derma_StringRequest("Select health", "What do you want the health of the person to be?", "",
				function(text)
					...
					RunConsoleCommand("_fadmin", "SetHealth", client:SteamID64(), health)
				end
			)
