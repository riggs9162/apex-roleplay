local function MuteChat(client, cmd, args)
	...
	for _, target in pairs(targets) do
		if not FAdmin.Access.PlayerHasPrivilege(client, "Chatmute", target) then FAdmin.Messages.SendMessage(client, 5, "No access!") return end
	...
	FAdmin.Messages.ActionMessage(client, targets, "You have chat muted %s " .. timeText, "Your chat was muted by %s " .. timeText, "Chat muted %s " .. timeText)
end

local function UnMuteChat(client, cmd, args)
	if not args[1] then return end
	...
	FAdmin.Messages.ActionMessage(client, targets, "You have chat unmuted %s", "Your chat was unmuted by %s", "Chat unmuted %s")
end
...
hook.Add("PlayerSay", "FAdmin_Chatmute", function(client, text, Team, dead)
	if client:FAdmin_GetGlobal("FAdmin_chatmuted") then return "" end
end)
