local function Slay(client, cmd, args)
	...
	for _, target in pairs(targets) do
		if not FAdmin.Access.PlayerHasPrivilege(client, "Slay", target) then FAdmin.Messages.SendMessage(client, 5, "No access!") return end
	...
	FAdmin.Messages.ActionMessage(client, targets, "Slayed %s", "You were slayed by %s", "Slayed %s")
end
