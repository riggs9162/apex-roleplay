...
FAdmin.StartHooks["zzSpectate"] = function()
	...
	-- Right click option
	FAdmin.ScoreBoard.Main.AddPlayerRightClick("Spectate", function(client)
		RunConsoleCommand("_FAdmin", "Spectate", client:SteamID64())
	end)

	-- Slap option in player menu
	FAdmin.ScoreBoard.Player:AddActionButton("Spectate", "FAdmin/icons/spectate", Color(0, 200, 0, 255), function(client) return FAdmin.Access.PlayerHasPrivilege(LocalPlayer(), "Spectate") and client != LocalPlayer() end, function(client)
		RunConsoleCommand("_FAdmin", "Spectate", client:SteamID64())
	end)
end
...
---------------------------------------------------------------------------*/
local function getThirdPersonPos(client)
...
---------------------------------------------------------------------------*/
local function specCalcView(client, origin, angles, fov)
...
local function spectateLookingAt()
	...
	for _, client in player.Iterator() do
		...
		local trace = util.QuickTrace(roamPos, pos - roamPos, client)

		if trace.Hit then continue end

		foundPly, foundDot = client, dot
	end

	if not IsValid(foundPly) then return end

	RunConsoleCommand("FAdmin", "Spectate", foundPly:SteamID64())
end
...
local keysDown = {}
local function specBinds(client, bind, pressed)
	if bind == "+jump" then
		stopSpectating()
