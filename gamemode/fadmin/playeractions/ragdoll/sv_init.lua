...
local function Ragdoll(client, cmd, args)
	...
	for _, target in pairs(targets) do
		if not FAdmin.Access.PlayerHasPrivilege(client, "Ragdoll", target) then FAdmin.Messages.SendMessage(client, 5, "No access!") return end
		if IsValid(target) then
			...
			elseif string.find(RagdollType, "kick") == 1 then -- Best ragdoll mod EVER
				...
				Kicker:SetModel("models/gman_high.mdl")
				Kicker:SetPos(kickerPos)
				Kicker:Spawn()
				Kicker:Activate()

				Kicker:EmitSound("gman/gman_choose1.wav", 100, 100)
	...
	if RagdollType == "unragdoll" or string.lower(cmd) == "unragdoll" then
		FAdmin.Messages.ActionMessage(client, targets, "Unragdolled %s", "You were unragdolled by %s", "Unragdolled %s")
	else
		FAdmin.Messages.ActionMessage(client, targets, "Ragdolled %s ".. timeText, "You were ragdolled by %s ".. timeText, "Ragdolled %s ".. timeText)
	end
end
...
hook.Add("PlayerSpawnObject", "FAdmin_ragdoll", function(client)
	if type(client.FAdminRagdoll) == "table" or IsValid(client.FAdminRagdoll) then
		return false
	end
end)

hook.Add("CanPlayerSuicide", "FAdmin_ragdoll", function(client)
	if type(client.FAdminRagdoll) == "table" or IsValid(client.FAdminRagdoll) then
		return false
	end
end)

hook.Add("PlayerDisconnected", "FAdmin_ragdoll", function(client)
	if not client.FAdminRagdoll then return end

	if IsValid(client.FAdminRagdoll) then
		client.FAdminRagdoll:Remove()
		return
	end

	if type(client.FAdminRagdoll) != "table" then return end

	for k,v in pairs(client.FAdminRagdoll or {}) do
