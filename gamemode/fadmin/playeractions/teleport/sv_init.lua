...
local function TPToPos(client, cmd, args)
	if not FAdmin.Access.PlayerHasPrivilege(client, "Teleport") then FAdmin.Messages.SendMessage(client, 5, "No access!") return end
	...
	client:SetPos(Vector(tonumber(x), tonumber(y), tonumber(z)))
	if vx and vy and vz then client:SetVelocity(Vector(tonumber(vx), tonumber(vy), tonumber(vz))) end
	zapEffect(client)
end

local function Teleport(client, cmd, args)
	if not FAdmin.Access.PlayerHasPrivilege(client, "Teleport") then FAdmin.Messages.SendMessage(client, 5, "No access!") return end
	...
	for _, target in pairs(targets) do
		if IsValid(target) and target:Alive() then
			...
			tracedata.endpos = tracedata.start + client:GetAimVector()*10000
			...
			local InitialPosition = GAMEMODE:FindEmptyPos(trace.HitPos + offset, {client}, 600, 20, Vector(16, 16, 64))
...
local function Bring(client, cmd, args)
	if not FAdmin.Access.PlayerHasPrivilege(client, "Teleport") then FAdmin.Messages.SendMessage(client, 5, "No access!") return end
	...
	BringTo = (BringTo and BringTo[1]) or client
	if not targets or #targets == 1 and not IsValid(targets[1]) then
...
local function Goto(client, cmd, args)
	if not FAdmin.Access.PlayerHasPrivilege(client, "Teleport") then FAdmin.Messages.SendMessage(client, 5, "No access!") return end
	if not args[1] then return end
	...
	client:SetPos(GAMEMODE:FindEmptyPos(target:GetPos(), {client}, 600, 30, Vector(16, 16, 64)))
	 FAdmin.Messages.SendMessage(client, 4, "You have teleported to: "..target:Name())
--	zapEffect(client)
end
