-- Kicking
local function Kick(client, cmd, args)
	...
	local CanKick = hook.Call("FAdmin_CanKick", nil, client, targets)
	...
	local Reason = (not table.HasValue(stages, stage) and table.concat(args, ' ', 2)) or table.concat(args, ' ', 3) or client.FAdminKickReason

	for _, target in pairs(targets) do
		if not FAdmin.Access.PlayerHasPrivilege(client, "Kick", target) then FAdmin.Messages.SendMessage(client, 5, "No access!")  return end
		if IsValid(target) then
			...
			elseif stage == "cancel" then
				...
				target:UnLock()
				target:Spawn()
				client.FAdminKickReason = nil
			elseif stage == "update" then -- Update reason text
				if not args[3] then return end
				client.FAdminKickReason = args[3]
				SendUserMessage("FAdmin_kick_update", target, args[3])
			else//if stage == "execute" or stage == "" then--execute or no stage = kick instantly
				local name = IsValid(client) and client:IsPlayer() and client:Nick() or "Console"

				FAdmin.Messages.ActionMessage(client, target, "You have kicked %s", "You were kicked by %s", "Kicked %s")
...
hook.Add("PlayerAuthed", "FAdmin_LeavingBeforeBan", function(client, SteamID, ...)
	if table.HasValue(StartBannedUsers, SteamID) then
		game.ConsoleCommand(string.format("kickid %s %s\n", client:UserID(), "Getting banned"))
	end
end)
...
local function RequestBans(client, cmd, args)
	if not FAdmin.Access.PlayerHasPrivilege(client, "UnBan") then FAdmin.Messages.SendMessage(client, 5, "No access!") return end
	net.Start("FAdmin_retrievebans")
		net.WriteTable(FAdmin.BANS)
	net.Send(client)
end

timer.Create("FAdminCheckBans", 2, 0, function()
...
local function Ban(client, cmd, args)
	...
	local CanBan = hook.Call("FAdmin_CanBan", nil, client, targets)

	if CanBan == false then return end

	local stage = string.lower(args[2])
	local stages = {"start", "cancel", "update", "execute"}
	local Reason = (not table.HasValue(stages, stage) and table.concat(args, ' ', 3)) or table.concat(args, ' ', 4) or client.FAdminKickReason

	for _, target in pairs(targets) do
		if (type(target) == "string" and not FAdmin.Access.PlayerHasPrivilege(client, "Ban")) or
		not FAdmin.Access.PlayerHasPrivilege(client, "Ban", target) then
			FAdmin.Messages.SendMessage(client, 5, "No access!")
			return
		end
		if stage == "start" and type(target) != "string" and IsValid(target) then
			...
			target:Lock() -- Make sure he can't remove the hook clientside and keep minging.
			target:KillSilent()
			table.insert(StartBannedUsers, target:SteamID64())
		...
		elseif stage == "update" then -- Update reason text
			...
			client.FAdminKickReason = args[4]
			...
			if type(target) != "string" and  IsValid(target) then
				...
				local nick = client:GetName() or "console"
				SaveBan(target:SteamID64(), target:Nick(), time, Reason, nick, client.SteamID and client:SteamID64() or "Console")

				FAdmin.Messages.ActionMessage(client, target, "You have Banned %s for " .. TimeText, "You were Banned by %s", "Banned %s (".. TimeText.. ") (".. Reason.. ")")
				--game.ConsoleCommand("banid " .. time.." ".. target:SteamID64().."\n") -- Don't use banid in combination with RunConsoleCommand
				...
				SaveBan(target, nil, time, Reason != "" and Reason, client:GetName() or "console", client.SteamID and client:SteamID64() or "Console") -- Again default to one hour
				--game.ConsoleCommand("banid ".. time.." ".. target.."\n")

				FAdmin.Messages.ActionMessage(client, {}, "You have Banned "..target.." for " .. TimeText, "", "Banned "..target.." (".. TimeText.. ") (".. Reason.. ")")
			end
			client.FAdminKickReason = nil
		end
	end
...
-- Unbanning
local function UnBan(client, cmd, args)
	if not FAdmin.Access.PlayerHasPrivilege(client, "UnBan") then FAdmin.Messages.SendMessage(client, 5, "No access!") return end
	if not args[1] then return end
	local SteamID = string.upper(args[1])

	hook.Call("FAdmin_UnBan", nil, client, SteamID)
	...
	FAdmin.Messages.ActionMessage(client, {}, "You have Unbanned "..SteamID, "", "Unbanned "..SteamID)
end
...
hook.Add("CheckPassword", "CustomBannedMessage", function( steamid64 )
	...
	if(FAdmin.BANS[steamid])then
		...
		if FAdmin.BANS[steamid].time != 0 then
			TimeText = os.date("%H:%M %x", FAdmin.BANS[steamid].time)
		end

		return false, [[Oh crumbs! You're banned:

		Reason: ]]..FAdmin.BANS[steamid].reason..[[

		Banned by: ]]..FAdmin.BANS[steamid].adminname..[[

		Banned until: ]]..TimeText..[[


		Make a unban appeal at http://apex-roleplay.com
		]]
...
concommand.Add("getInfo", function(client)
	-- body
end)
