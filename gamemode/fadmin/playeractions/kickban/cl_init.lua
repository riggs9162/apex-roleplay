...
usermessage.Hook("FAdmin_kick_cancel", function()
	...
	LocalPlayer():Notify("Kick was canceled by admin")
end)
...
usermessage.Hook("FAdmin_ban_cancel", function()
	...
	LocalPlayer():Notify("Ban was canceled by admin")
end)
...
FAdmin.StartHooks["CL_KickBan"] = function()
	...
	FAdmin.ScoreBoard.Main.AddPlayerRightClick("Quick Kick", function(client, Panel)
		RunConsoleCommand("_FAdmin", "kick", client:SteamID64(), "Quick kick")
		if ValidPanel(Panel) then Panel:Remove() end
	end)


	-- Kick button
	FAdmin.ScoreBoard.Player:AddActionButton("Kick", "FAdmin/icons/kick", nil, function(client) return FAdmin.Access.PlayerHasPrivilege(LocalPlayer(), "Kick", client) end, function(client)
	...
	-- Ban button
	FAdmin.ScoreBoard.Player:AddActionButton("Ban", "FAdmin/icons/Ban", nil, function(client) return FAdmin.Access.PlayerHasPrivilege(LocalPlayer(), "Ban", client) end, function(client)
		local SteamID = client:SteamID64()
		local NICK = client:Nick()

		if client:IsBot() then SteamID = "BOT" end
