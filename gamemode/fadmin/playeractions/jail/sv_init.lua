local function Jail(client, cmd, args)
	...
	for _, target in pairs(targets) do
		if not FAdmin.Access.PlayerHasPrivilege(client, "Jail", target) then FAdmin.Messages.SendMessage(client, 5, "No access!") return end
	...
	if JailType == "unjail" or string.lower(cmd) == "unjail" then
		FAdmin.Messages.ActionMessage(client, targets, "Unjailed %s", "You were unjailed by %s", "Unjailed %s")
	else
		FAdmin.Messages.ActionMessage(client, targets, "Jailed %s in a "..JailType.." jail ".. time, "You were jailed " .. time .. " by %s", "Jailed %s for "..time)
	end
end
...
hook.Add("PlayerSpawn", "FAdmin_jail", function(client)
	if client:FAdmin_GetGlobal("fadmin_jailed") then
		timer.Simple(0.1, function() if IsValid(client) then client:SetPos(client.FAdminJailPos) end end)
	end
end)

hook.Add("PlayerNoClip", "FAdmin_jail", function(client)
	if client:FAdmin_GetGlobal("fadmin_jailed") then
		return false
	end
end)

hook.Add("PlayerSpawnObject", "FAdmin_jailed", function(client)
	if client:FAdmin_GetGlobal("fadmin_jailed") then
		return false
	end
end)