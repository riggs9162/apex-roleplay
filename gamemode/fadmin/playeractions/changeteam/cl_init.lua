FAdmin.StartHooks["zzSetTeam"] = function()
	...
	function(client) return FAdmin.Access.PlayerHasPrivilege(LocalPlayer(), "SetTeam", client) end, function(client, button)
		...
		for k,v in SortedPairsByMemberValue(team.GetAllTeams(), "Name") do
			menu:AddOption(v.Name, function() RunConsoleCommand("_FAdmin", "setteam", client:SteamID64(), k) end)
		end
		menu:Open()
	end)
end