local function SetTeam(client, cmd, args)
	...
	for k,v in pairs(team.GetAllTeams()) do
		if k == tonumber(args[2]) or string.lower(v.Name) == string.lower(args[2] or "") then
			for _, target in pairs(targets) do
				if not FAdmin.Access.PlayerHasPrivilege(client, "SetTeam", target) then FAdmin.Messages.SendMessage(client, 5, "No access!") return end
				local SetTeam = target.ChangeTeam or target.SetTeam -- DarkRP compatibility
				if IsValid(target) then
					SetTeam(target, k, true)
					end
			end
			FAdmin.Messages.ActionMessage(client, targets, "You have set the team of %s", "Your team was set to "..v.Name.." by %s", "Set the team of %s")
			break
		end
	end
end
