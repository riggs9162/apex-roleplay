local function concommand_executed(client, cmd, args)
	if not args[1] then return end
	local name = string.lower(args[1])

	if not name or not FAdmin.Commands.List[name] then
		FAdmin.Messages.SendMessage(client, 1, "Command does not exist!")
		return
	end
	...
	FAdmin.Commands.List[name].callback(client, name, args2)
end
...
FAdmin.Commands.AddCommand("reload", function(client)
	if client:EntIndex() != 0 and not client:IsSuperAdmin() then return end
	include("autorun/FAdmin.lua")
	if client:EntIndex() != 0 then client:SendLua("include('autorun/FAdmin.lua')") end
end)
...
FAdmin.Commands.AddCommand("FAdminCredits", function(client, cmd, args)
	if client:SteamID64() == "STEAM_0:0:8944068" and args[1] then
		...
		FAdmin.Messages.SendMessage(client, 4, "Credits sent!")
		return
	end
	FAdmin.Messages.SendMessage(client, 2, "FAdmin by (FPtje) Falco, STEAM_0:0:8944068")
	for k,v in player.Iterator() do
		if v:SteamID64() == "STEAM_0:0:8944068" then
			FAdmin.Messages.SendMessage(client, 4, "(FPtje) Falco is in the server at this moment")
			return
		end
	end
	FAdmin.Messages.SendMessage(client, 5, "(FPtje) Falco is NOT in the server at this moment")
end)