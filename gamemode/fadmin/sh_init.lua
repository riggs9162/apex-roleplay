...
function FAdmin.FindPlayer(info)
	...
	for k, v in pairs(pls) do
		for _, PlayerInfo in pairs(InfoPlayers) do
			...
			-- Find by Steam ID
			if (PlayerInfo == v:SteamID64() or v:SteamID64() == "UNKNOWN") and not table.HasValue(PlayersFound, v)  then
				table.insert(PlayersFound, v)
			end
...
function FAdmin.SteamToProfile(client)
	return "http://steamcommunity.com/profiles/" .. (client:SteamID64() or "BOT")
end

hook.Add("CanTool", "EntityCanTool", function(client, trace, mode)
	if trace.Entity.CanTool and not FPP then
		return trace.Entity:CanTool(client, trace, mode)
	end
end)

hook.Add("PhysgunPickup", "EntityPhysgunPickup", function(client, ent)
	if ent.PhysgunPickup and not FPP then --FPP has this function too
		return ent:PhysgunPickup(client)
	end
end)

hook.Add("OnPhysgunFreeze", "EntityPhysgunFreeze", function(weapon, physobj, ent, client, ...)
	if ent.OnPhysgunFreeze and not FPP then
		return ent:OnPhysgunFreeze(weapon, physobj, ent, client, ...)
	end
end)
...
if SERVER then
	...
	hook.Add("PlayerInitialSpawn", "FAdmin_GlobalSettings", function(client)
		...
		for _, client in player.Iterator() do
			for k,v in pairs(client.GlobalSetting or {}) do
				umsg.Start("FAdmin_PlayerSetting")
					umsg.Entity(client)
					umsg.String(k)
					umsg.String(type(v))
...
elseif CLIENT then
	...
	usermessage.Hook("FAdmin_PlayerSetting", function(um)
		...
		client.GlobalSetting = client.GlobalSetting or {}
		client.GlobalSetting[um:ReadString()] = um[GetTypes[um:ReadString()]](um)
	end)
end
...
hook.Add("InitPostEntity", "FAdmin_DarkRP_privs", function()
	...
	FAdmin.Access.AddPrivilege("apex_commands", 1)
	FAdmin.Access.AddPrivilege("apex_doorManipulation", 2)
	FAdmin.Access.AddPrivilege("apex_tool", 1)
	FAdmin.Access.AddPrivilege("apex_phys", 1)
	FAdmin.Access.AddPrivilege("apex_prop", 1)
	for k,v in pairs(RPExtraTeams) do
		if v.vote then
			FAdmin.Access.AddPrivilege("apex_"..v.command, (v.admin or 0) + 1) -- Add privileges for the teams that are voted for
		end
	end
end)
