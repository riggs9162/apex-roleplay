...
local function DoRestrictWeapons(client, cmd, args)
	if not FAdmin.Access.PlayerHasPrivilege(client, "Restrict") then print("no1") FAdmin.Messages.SendMessage(client, 5, "No access!") return end
	local Weapon = args[1]
	local Group = args[2]
	...
	FAdmin.Messages.SendMessage(client, 4, "Weapon restricted!")
end

local function RestrictWeapons(client, Weapon, WeaponTable)
	local Group = client:GetNWString("usergroup")
...
FAdmin.StartHooks["Restrict"] = function()
	...
	FAdmin.Access.AddPrivilege("Restrict", 2)
end