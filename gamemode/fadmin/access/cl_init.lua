...
local function addGroupUI(client, func)
	...
	function(text)
		...
		Derma_Query("On what access will this team be based? (the new group will inherit all the privileges from the group)", "Admin access",
			"user", function() ContinueNewGroup(client, text, 0, func) end,
			"admin", function() ContinueNewGroup(client, text, 1, func) end,
			"superadmin", function() ContinueNewGroup(client, text, 2, func) end)
	end)
end
...
FAdmin.StartHooks["1SetAccess"] = function() -- 1 in hook name so it will be executed first.
	...
	function(client) return FAdmin.Access.PlayerHasPrivilege(LocalPlayer(), "SetAccess") or LocalPlayer():IsSuperAdmin() end, function(client)
		...
		for k,v in SortedPairsByMemberValue(FAdmin.Access.Groups, "ADMIN", true) do
			menu:AddOption(k, function()
				...
				RunConsoleCommand("_FAdmin", "setaccess", client:SteamID64(), k)
			end)
		end

		menu:AddOption("New...", function() addGroupUI(client) end)
		menu:Open()
	end)
	...
	FAdmin.ScoreBoard.Server:AddServerSetting(function()
		...
		end, Color(0, 0, 155, 255), function(client) return FAdmin.Access.PlayerHasPrivilege(LocalPlayer(), "SetAccess") end, function(button)
...
ContinueNewGroup = function(client, name, admin_access, func)
	local privs = {}
	...
	function OKButton:DoClick()
		if client then
			RunConsoleCommand("_FAdmin", "setaccess", client:SteamID64(), name, admin_access, unpack(privs))
		else
