...
function FAdmin.Access.RemoveGroup(client, cmd, args)
	...
	if FAdmin.Access.Groups[args[1]] and not table.HasValue({"superadmin", "admin", "user", "noaccess"}, string.lower(args[1])) then
		...
		FAdmin.Messages.SendMessage(client, 4, "Group succesfully removed")
...
function FAdmin.Access.PlayerHasPrivilege(client, priv, target)
	-- This is the server console
	if client:EntIndex() == 0 or game.SinglePlayer() or (client.IsListenServerHost and client:IsListenServerHost()) then return true end
	-- Privilege does not exist
	if not FAdmin.Access.Privileges[priv] then return client:IsAdmin() end

	local Usergroup = client:GetNWString("usergroup")

	if FAdmin.GlobalSetting.Immunity and type(target) != "string" and IsValid(target) and target != client and FAdmin.Access.Groups[Usergroup] and
	...
	if CLIENT and client.FADMIN_PRIVS and client.FADMIN_PRIVS[priv] then return true end
...
FAdmin.StartHooks["AccessFunctions"] = function()
	...
	FAdmin.Commands.AddCommand("Admins", function(client)
		if not FAdmin.Access.PlayerHasPrivilege(client, "SeeAdmins") then return end
		for k,v in player.Iterator() do
			client:PrintMessage(HUD_PRINTCONSOLE, v:Nick() .. "\t|\t" .. v:GetNWString("usergroup"))
		end
	end
	)
end
