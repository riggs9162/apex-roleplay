...
hook.Add("DatabaseInitialized", "InitializeFAdminGroups", function()
	...
	MySQL.query("CREATE TABLE IF NOT EXISTS FAdmin_PlayerGroup(steamid VARCHAR(40) NOT NULL, groupname VARCHAR(40) NOT NULL, PRIMARY KEY(steamid));")
	...
	MySQLite.queryValue("SELECT COUNT(*) FROM FADMIN_PRIVILEGES;", function(val)
		...
		for priv, access in pairs(FAdmin.Access.Privileges) do
			for i = access + 1, #hasPrivs, 1 do
				...
				MySQL.query("INSERT INTO FADMIN_PRIVILEGES VALUES(" .. MySQL.SQLStr(hasPrivs[i]) .. ", " .. MySQL.SQLStr(priv) .. ");")
			end
		end
	end)
end)

function FAdmin.Access.PlayerSetGroup(client, group)
	if not FAdmin.Access.Groups[group] then return end
	client = isstring(client) and FAdmin.FindPlayer(client) and FAdmin.FindPlayer(client)[1] or client
	local SteamID = type(client) != "string" and IsValid(client) and client:SteamID64() or client

	if type(client) != "string" and IsValid(client) then
		client:SetUserGroup(group)
	end

	MySQL.query("REPLACE INTO FAdmin_PlayerGroup VALUES(" .. MySQL.SQLStr(SteamID)..", " .. MySQL.SQLStr(group)..");")
end

function FAdmin.Access.SetRoot(client, cmd, args) -- FAdmin setroot player. Sets the player to superadmin
	if not FAdmin.Access.PlayerHasPrivilege(client, "SetAccess") then FAdmin.Messages.SendMessage(client, 5, "No access!") return end
	...
	for _, target in pairs(targets) do
		if IsValid(target) then
			...
			if ULib and ULib.ucl and ULib.ucl.groups and ULib.ucl.groups["superadmin"] then --Add to ULX
				ULib.ucl.addUser(target:SteamID64(), nil, nil, "superadmin")
			end
			FAdmin.Messages.SendMessage(client, 2, "User set to superadmin!")
		end
	end
end
...
local function AddGroup(client, cmd, args)
	...
	FAdmin.Messages.SendMessage(client, 4, "Group created")
	FAdmin.Access.SendGroups()
end

local function AddPrivilege(client, cmd, args)
	if not FAdmin.Access.PlayerHasPrivilege(client, "SetAccess") then FAdmin.Messages.SendMessage(client, 5, "No access!") return end

	local group, priv = args[1], args[2]
	if not FAdmin.Access.Groups[group] or not FAdmin.Access.Privileges[priv] then
		FAdmin.Messages.SendMessage(client, 5, "Invalid arguments")
		return
	end

	FAdmin.Access.Groups[group].PRIVS[priv] = true

	MySQLite.query("REPLACE INTO FADMIN_PRIVILEGES VALUES(" .. MySQL.SQLStr(group) .. ", " .. MySQL.SQLStr(priv) .. ");")
	SendUserMessage("FAdmin_AddPriv", player.GetAll(), group, priv)
	FAdmin.Messages.SendMessage(client, 4, "Privilege Added!")
end

local function RemovePrivilege(client, cmd, args)
	if not FAdmin.Access.PlayerHasPrivilege(client, "SetAccess") then FAdmin.Messages.SendMessage(client, 5, "No access!") return end
	...
	MySQLite.query("DELETE FROM FADMIN_PRIVILEGES WHERE NAME = " .. MySQL.SQLStr(group) .. " AND PRIVILEGE = " .. MySQL.SQLStr(priv) .. ";")
	SendUserMessage("FAdmin_RemovePriv", player.GetAll(), group, priv)
	FAdmin.Messages.SendMessage(client, 4, "Privilege Removed!")
end

function FAdmin.Access.SendGroups(client)
...
function FAdmin.Access.SetAccess(client, cmd, args)
	if not FAdmin.Access.PlayerHasPrivilege(client, "SetAccess") then FAdmin.Messages.SendMessage(client, 5, "No access!") return end

	local targets = FAdmin.FindPlayer(args[1])

	if not args[2] or (not FAdmin.Access.Groups[args[2]] and not tonumber(args[3])) then
		FAdmin.Messages.SendMessage(client, 1, "Group not found")
		return
	elseif args[2] and not FAdmin.Access.Groups[args[2]] and tonumber(args[3]) then
		...
		FAdmin.Messages.SendMessage(client, 2, "Group created")
		FAdmin.Access.SendGroups()
	end
	...
	if not targets and (string.find(args[1], "STEAM_") or args[1] == "BOT") then
		...
		FAdmin.Messages.SendMessage(client, 4, "User access set!")
		return
...
hook.Add("PlayerInitialSpawn", "FAdmin_SetAccess", function(client)
	MySQL.queryValue("SELECT groupname FROM FAdmin_PlayerGroup WHERE steamid = " .. MySQL.SQLStr(client:SteamID64())..";", function(Group)
		if not Group then return end
		client:SetUserGroup(Group)

		if FAdmin.Access.Groups[Group] then
			client:FAdmin_SetGlobal("FAdmin_admin", FAdmin.Access.Groups[Group].ADMIN_ACCESS)
		end
	end)
	FAdmin.Access.SendGroups(client)
	--timer.Simple( 5, function() hook.Call("slotsStuff", nil, client) end )
	
end)

local function SetImmunity(client, cmd, args)
	-- SetAccess privilege because they can handle immunity settings
	if not FAdmin.Access.PlayerHasPrivilege(client, "SetAccess") then FAdmin.Messages.SendMessage(client, 5, "No access!") return end

	if not args[1] then FAdmin.Messages.SendMessage(client, 5, "Invalid argument!") return end
	RunConsoleCommand("_FAdmin_immunity", args[1])
	FAdmin.Messages.SendMessage(client, 4, "turned " .. ((tonumber(args[1]) == 1 and "on") or "off") .. " admin immunity!")
end
