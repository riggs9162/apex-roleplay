TEAM_CITIZEN = AddExtraTeam("Citizen", {
    ...
    They are forced to follow the Combine’s dictatorship with absolute 
    obedience, or face punishments and even execution. The Combine keeps
    ...
    xp = 0,
                ...
                PlayerLoadout = function(client) client:SetArmor(0) return true end,
    customCheck = function(client)
    if SERVER then
        client:SetDarkRPVar("citopt", 1)
    end
        return true
    end
})
...
TEAM_CWU = AddExtraTeam("Civil Worker's Union", {
    ...
    to work for the Combine. CWU receive many benefits, with access to
...
XP Requirement: 15]],
    ...
    mayorCanSetSalary = false,
    xp = 15,
    PlayerDeath = function(client, weapon, killer)
        client:ChangeTeam(GAMEMODE.DefaultTeam, true)
    end,
    customCheck = function(client)
    if SERVER then
        client:SetDarkRPVar("citopt", 3)
    end
        return true
    end
})
...
TEAM_CP = AddExtraTeam("Civil Protection", {
    ...
    description = [[The CPs are the Combine’s human police force.
    They are responsible for the enforcement of the Combine’s laws, and 
...
TEAM_OVERWATCH = AddExtraTeam("Overwatch Transhuman Arm", {
    ...
    description = [[The Overwatch Transhuman Arm are the military wing of the Combine’s forces.
    ...
    regard to their own safety. Operating in small squads, the Overwatch Transhuman Arm are a force
...
TEAM_ADMINISTRATOR = AddExtraTeam("City Administrator", {
    ...
    to run the city by the Combine. He has been chosen because of
    his fierce support of and loyalty for the Combine. They are given a great deal
...
TEAM_VORT = AddExtraTeam("Vortigaunt", {
    ...
    description = [[A mysterious alien race, enslaved by the Combine.
...
XP Requirement: 1200]],
    ...
    xp = 1200,
    mayor = false,
    customCheck = function(client) return
        table.HasValue({"admin", "superadmin", "moderator", "vip", "developer"}, client:GetNWString("usergroup"))
    end,
    CustomCheckFailMsg = "You can only become a Vortigaunt if you donate, type !vip",
    PlayerCanPickupWeapon = function(client, weapon) return false end,
    PlayerLoadout = function(client)
        client:ChatPrint( "You selected vortigaunt, please roleplay correctly or you will be banned." )
        client:ChatPrint( "Playing as this job requires a high standard of RP." )
        client:ChatPrint( "Do not use your microphone." )
        client:ChatPrint( "Do not change your name." )
        client:ChatPrint( "Feel free to use OOC tools in order to RP properly" )
        client:ChatPrint( "This role cannot pickup weapons." )
        return true
    end,
})