...
hook.Add("CalcView", "DrunkVision", function(client, origin, angle, fov)

	local netdrunklevel = client:GetDarkRPVar( "DrunkLevel")
...
hook.Add("CreateMove", "DrunkWalk", function(move)
	...
	if !IsValid(client) or drunklevel <= 0 or LocalPlayer():GetMoveType() == MOVETYPE_NOCLIP then return end
...
net.Receive( "DrunkVomit", function( len, client )
	puke = CurTime();
