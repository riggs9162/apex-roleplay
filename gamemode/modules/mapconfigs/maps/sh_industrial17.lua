apex.mapconfig.Register("rp_apex_industrial17_b10", {
	APCSpawnPos = Vector(1535, 5817, 144),
	APCSpawnAng = Angle(0, -180, 0),
	...
	ButtonTable = {
		{
			...
			check = function(client)
				if ( client:CanAfford(1) ) then
					client:AddMoney(-1)
					client:Notify("You put 1 token into the vending machine.")
					return true
				else
					client:Notify("You don't have enough money to make this purchase.")
					return false
				end
			end
		},
		{
			...
			check = function(client)
				if ( client:IsCombine() or client:IsAdmin() ) then
					if ( LastJw and LastJw + 15 > CurTime() ) then
						return false
					end

					local name = client:Name()
					if ( string.match(name, "SeC") or string.match(name, "DvL") or string.match(name, "OWC") or client:Team() == TEAM_ADMINISTRATOR or client:IsAdmin() ) then
						client:Notify("Judgement Waiver Activated.")
						SetGlobalBool("jw", true)
						LastJw = CurTime()
						return true
					else
						client:Notify("Access denied to Judgment Waiver Controls")
						return false
					end
				end
			end
		},
		{
			...
			check = function(client)
				if ( client:IsCombine() or client:IsAdmin() ) then
					...
					local name = client:Name()
					if ( string.match(name, "SeC") or string.match(name, "DvL") or string.match(name, "OWC") or client:Team() == TEAM_ADMINISTRATOR or client:IsAdmin() ) then
						client:Notify("Judgement Waiver De-activated.")
		...
		},
		{
			...
			check = function(client)
				if ( client:IsAdmin() ) then
					...
					client:Notify("Lockdown de-activated.")
					return true
				else
					client:Notify("Access denied to Lockdown controls.")
					return false
				end
			end
		},
		{
			...
			check = function(client)
				if ( client:IsAdmin() ) then
					...
					client:Notify("Lockdown activated.")
					return true
				else
					client:Notify("Access denied to Lockdown controls.")
					return false
		...
		},
		{
			...
			check = function(client)
				local name = client:Name()
				if ( string.match(name, "SeC") or string.match(name, "DvL") or string.match(name, "OWC") or client:Team() == TEAM_ADMINISTRATOR or client:IsAdmin() ) then
					client:Notify("Gate toggled.")
					return true
				else
					client:Notify("Access denied to gate controls.")
					return false
				end
			end
		},
		...
		},
		{
			...
			check = function(client)
				local name = client:Name()
				if ( string.match(name, "SeC") or string.match(name, "DvL") or string.match(name, "OWC") or client:Team() == TEAM_ADMINISTRATOR or client:IsAdmin() ) then
					client:Notify("Shell beach gate opened.")
					return true
				else
					client:Notify("Access denied to Shell beach gate controls.")
					return false
				end
			end
		},
		{
			...
			check = function(client)
				local name = client:Name()
				if ( string.match(name, "SeC") or string.match(name, "DvL") or string.match(name, "OWC") or client:Team() == TEAM_ADMINISTRATOR or client:IsAdmin() ) then
					client:Notify("Shell beach gate closed.")
					return true
				else
					client:Notify("Access denied to Shell beach gate controls.")
					return false
		...
		},
		{
			...
			check = function(client)
				local name = client:Name()
				if ( string.match(name, "SeC") or string.match(name, "DvL") or string.match(name, "OWC") or client:Team() == TEAM_ADMINISTRATOR or client:IsAdmin() ) then
					client:Notify("Access granted to RDC door")
					return true
				else
					client:Notify("Access denied to RDC door")
					return false
				end
			end
		},
		{
			...
			check = function(client)
				local name = client:Name()
				if ( string.match(name, "SeC") or string.match(name, "DvL") or string.match(name, "OWC") or client:Team() == TEAM_ADMINISTRATOR or client:IsAdmin() ) then
					client:Notify("Access granted to RDC closed")
					return true
				else
					client:Notify("Access denied to RDC closed")
					return false
				end
			end
		},
		{
			...
			check = function(client)
				local name = client:Name()
				if ( string.match(name, "SeC") or string.match(name, "DvL") or string.match(name, "OWC") or client:Team() == TEAM_ADMINISTRATOR or client:IsAdmin() ) then
					client:Notify("Access granted to workcycle announcment")
					return true
				else
					client:Notify("Access denied to workcycle announcment")
					return false
				end
			end
		},
		...
		},
		{
			...
			check = function(client)
				local name = client:Name()
				if ( string.match(name, "SeC") or string.match(name, "OWC") or string.match(name, "DvL") or client:Team() == TEAM_ADMINISTRATOR or client:IsAdmin() ) then
					client:Notify("Access granted to RDC open")
					return true
				else
					client:Notify("Access denied to RDC open")
					return false
				end
			end
		}
