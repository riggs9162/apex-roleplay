...
maphl2config["apex_nc_city8_v2b"] = {
...
check = function(p)
	if p:IsCombine() or p:IsAdmin() then
		if string.match( p:Name(), "CPSeC" ) or string.match( p:Name(), "CPCmD" ) or string.match( p:Name(), "OTA-OWC" ) or p:Team() == TEAM_ADMINISTRATOR or p:IsAdmin() then
...
check = function(p)
	if p:IsCombine() or p:IsAdmin() then
	if string.match( p:Name(), "CPSeC" ) or string.match( p:Name(), "CPCmD" ) or string.match( p:Name(), "DvL" ) or p:Team() == TEAM_OVERWATCH or  p:Team() == TEAM_ADMINISTRATOR or p:IsAdmin() then
